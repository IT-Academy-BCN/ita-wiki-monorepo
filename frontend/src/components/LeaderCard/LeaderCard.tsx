...
const LeaderCard: FC<{ player: LeaderCardPlayer; cupType: CupType }> = ({ player, cupType }) => {
  ...
  return (
    <div className="flex flex-col items-center gap-2 bg-white rounded-xl border border-gray-200 px-6 py-4 shadow-sm min-w-[180px]">
      <span className={clsx('text-2xl', text)} aria-label={`${cupType} cup`}>{icon}</span>
      <div className={clsx('w-14 h-14 rounded-full border-2 flex items-center justify-center overflow-hidden', border)}>
        {!imgError && player.avatarUrl ? (
          <img src={player.avatarUrl} alt={`${player.username} avatar`} className="w-full h-full object-cover" onError={() => setImgError(true)} />
        ) : (
          <span className="text-sm font-bold text-gray-600">{player.username.slice(0, 2).toUpperCase()}</span>
        )}
      </div>
