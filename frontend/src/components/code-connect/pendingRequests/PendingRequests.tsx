...
const PendingRequests = ({ projectId, ownerId, currentUserId }: PendingRequestsProps) => {
  ...
  useEffect(() => {
    fetchProjectContributors(projectId).then(setContributors).catch(() => {});
  }, [projectId, tick]);
  ...
  return (
    <div className="mb-8 p-4 bg-pink-50 rounded-lg">
      ...
      <ul className="flex flex-col gap-3">
        {pending.map((c) => (
          <li key={c.id} className="flex items-center justify-between gap-4 p-3 border rounded-lg">
            <span className="font-semibold">[{c.user.name}]</span>
            <span className="text-sm text-gray-500">{c.programming_role}</span>
            <div className="flex gap-2">
              <button onClick={() => void handleAction(c.id, "accepted")} className="px-3 py-1 border border-gray-400 text-gray-700 rounded hover:bg-gray-100 text-sm">
                Acceptar
              </button>
              <button onClick={() => void handleAction(c.id, "rejected")} className="px-3 py-1 border border-gray-400 text-gray-700 rounded hover:bg-gray-100 text-sm">
                Rebutjar
              </button>
            </div>
          </li>
