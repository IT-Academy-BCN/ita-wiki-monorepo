...
class LigaPromotionTest extends TestCase
{
    ...
    private function getLeagueIdValue(Liga $liga): int
    {
        $leagueId = $liga->league_id;
        return $leagueId instanceof \BackedEnum ? $leagueId->value : $leagueId;
    }
    ...
    public function test_top_3_users_get_promoted(): void {
        ...
        $this->assertEquals(2, $this->getLeagueIdValue(Liga::where('user_id', $users[0]->id)->first()));
        $this->assertEquals(2, $this->getLeagueIdValue(Liga::where('user_id', $users[1]->id)->first()));
        $this->assertEquals(2, $this->getLeagueIdValue(Liga::where('user_id', $users[2]->id)->first()));
    }
    ...
    public function test_worst_3_users_get_demoted(): void {
        ...
        $this->artisan('liga:process-promotions')->assertSuccessful();
        $this->assertEquals(1, $this->getLeagueIdValue(Liga::where('user_id', $users[6]->id)->first()));
        $this->assertEquals(1, $this->getLeagueIdValue(Liga::where('user_id', $users[7]->id)->first()));
        $this->assertEquals(1, $this->getLeagueIdValue(Liga::where('user_id', $users[8]->id)->first()));
    }
    ...
    public function test_user_in_gold_cannot_be_promoted(): void {
        ...
        $this->artisan('liga:process-promotions')->assertSuccessful();
        $this->assertEquals(3, $this->getLeagueIdValue(Liga::where('user_id', $users[0]->id)->first()));
    }
    ...
    public function test_user_in_bronze_cannot_be_demoted(): void {
        ...
        $this->artisan('liga:process-promotions')->assertSuccessful();
        $this->assertEquals(1, $this->getLeagueIdValue(Liga::where('user_id', $users[2]->id)->first()));
    }
    ...
    public function test_league_with_less_than_3_users_is_skipped(): void {
        ...
        Liga::create(['user_id' => $users[1]->id, 'points_weekly' => 50,  'league_id' => 1]);
        $this->artisan('liga:process-promotions')->assertSuccessful();
        $this->assertEquals(1, $this->getLeagueIdValue(Liga::where('user_id', $users[0]->id)->first()));
        $this->assertEquals(1, $this->getLeagueIdValue(Liga::where('user_id', $users[1]->id)->first()));
    }
}